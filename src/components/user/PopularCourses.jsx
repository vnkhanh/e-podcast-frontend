        ...
        <Row gutter={[24, 24]}>
          {courses.map((course, index) => (
            <Col xs={24} sm={12} lg={6} key={index}>
              ...
              >
                <Title level={4} style={{ marginBottom: 8, fontSize: 16 }}>{course.title}</Title>
                <Text type="secondary" style={{ display: 'block', marginBottom: 12, fontSize: 14 }}>
                  Bởi {course.instructor}
                </Text>
                
                <div style={{ display: 'flex', alignItems: 'center', marginBottom: 12 }}>
                  <Text strong style={{ color: '#ffa940', fontSize: 16 }}>{course.rating}</Text>
                  <Rate disabled defaultValue={course.rating} style={{ fontSize: 14, margin: '0 8px' }} />
                ...
                {course.progress > 0 ? (
                  <div style={{ marginBottom: 16 }}>
                    <Progress percent={course.progress} size="small" status="active" />
