...
import { Table, Button, Space, Input, Select, message, Upload, Descriptions, Modal, Popconfirm } from "antd";
import { EyeOutlined, UploadOutlined, DeleteOutlined } from "@ant-design/icons";
import { listDocuments, uploadDocument, getDocumentDetail, deleteDocument } from "../../../services/api_document";
...
const DocumentPage = () => {
 ...
 useAutoWebSocket(socketUrl, (event) => {
  ...
  try {
        ...
        console.log("Non-JSON WS message:", raw);
        console.log("Lỗi: ", err);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          <Button icon={<EyeOutlined/>} type="primary" onClick={() => fetchDocumentDetail(record.id)}></Button>
           <Popconfirm
            title="Xoá tài liệu?"
